...
async def test_importer_creates_book_from_epub(session: AsyncSession, library_path: Path) -> None:
    ...
    assert imported_book.title  # non-empty
    assert any("swift" in author.name.lower() for author in imported_book.authors)
    assert len(imported_book.formats) == 1
    assert imported_book.formats[0].file_hash  # set

    # File placed in library layout
...
@pytest.mark.asyncio
async def test_importer_rejects_unknown_extension(
    session: AsyncSession, library_path: Path
) -> None:
    """An unsupported extension raises UnsupportedFormatError."""
    from shelvr.formats.base import UnsupportedFormatError
    from shelvr.services.importer import import_file

    with pytest.raises(UnsupportedFormatError):
        await import_file(
            file_bytes=b"not an ebook",
